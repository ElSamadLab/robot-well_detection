...
fcs <- read.fcs(file.name)#FCS(file.name), transformation=FALSE)
times = exprs(fcs)[,"Time"]
s = 100; # Convert to seconds
counts = hist(times,breaks=seq(from=min(times)-s,to=max(times)+s,by=s))$counts # per second
...
well.splits.starts = list()
well.splits.ends = list()
for( i in 1:length(plate.splits$start)){
    ...
    well.splits.starts[[i]]=well.splits$start
    well.splits.ends[[i]]=well.splits$end
}
