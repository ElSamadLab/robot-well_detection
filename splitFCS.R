...
for( i in 1:length(plate.splits$start)){
    well.splits = split.fcs(counts[plate.splits$start[i]:plate.splits$end[i]], smooth=1, reasonable.time.min=5, reasonable.time.max=14, 10, plot=T, title=paste('plate',toString(i)))
    plate[[i]] = list()
